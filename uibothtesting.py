...
def CalculateButton(frame):
    ...
    calculateButton.pack(side=tk.LEFT, anchor='e', expand=True, padx=20)#grid(row=4, column=1, padx = 10, pady=5, sticky='w')
...
def CreateNotebook():
    def ChangedTab(event):
      ...
      print("Selected tab index:", selectedTab)
...
def CreateShared():
    ...
    leftFrame = tk.Frame(mainFrame, bd=1)#, relief=tk.SUNKEN
    ...
    VerboseCheckboxSettings(leftFrame)
    AppplyPatchCheckbox(leftFrame)
    ModifyBaseCVSS(leftFrame)
    ### CalculateButton(leftLeftFrame)
    #test(leftLeftFrame)
    ...
    CalculateButton(bottomFrame)
    bottomFrame.pack(side=tk.BOTTOM, padx=10, pady=5, fill=tk.BOTH, expand=True, anchor='center')
